...
const GESTURE_NAMES: Record<string, string> = { 'fist': '握拳', 'open_palm': '张开手掌', 'peace': '比耶', 'point': '食指', 'pinch': '捏合' }
...
class App {
  ...
  // ===== 光标状态机（新增）=====
  private cursorState: 'hidden' | 'tracking' | 'armed' | 'dwelling' | 'locked' | 'suspended' = 'hidden'
  private cameraControlActive = false
  private cameraControlTimeout: number | null = null
  ...
  constructor() {
    ...
    try {
      ...
      this.handTracker.onPointerUpdate((pointer: PointerUpdate) => {
        ...
        // ===== 相机仲裁：占用时挂起 dwell，光标半透明 =====
        if (this.cameraControlActive) {
          if (this.cursorState !== 'suspended') this.applyCursorState('suspended')
          this.resetDwell()
          return
        }
        // ===== 根据 armed 状态驱动光标状态机 =====
        if (pointer.armed) {
          if (this.cursorState !== 'dwelling' && this.cursorState !== 'locked') {
            this.applyCursorState('armed')
          }
        } else {
          if (this.cursorState !== 'dwelling' && this.cursorState !== 'locked') {
            this.applyCursorState('tracking')
          }
        }
        // ===== Dwell 更新：peace + armed → 停留 650ms 自动选中 =====
        if (pointer.stableGesture === 'peace' && pointer.armed) {
          this.updateDwell(pointer.screenX, pointer.screenY)
        } else {
          this.resetDwell()
        }
      })
      ...
      this.handTracker.onPointerLost(() => {
        ...
        if (this.cameraControlTimeout !== null) {
          clearTimeout(this.cameraControlTimeout)
          this.cameraControlTimeout = null
        }
        this.resetDwell()
        this.applyCursorState('hidden')
      })
  ...
  private buildShapePanel() {
    ...
    for (const [category, shapes] of Object.entries(SHAPE_CATEGORIES)) {
      html += `<div class="shape-category">`
      html += `<div class="category-name">${category}</div>`
      html += `<div class="shape-list">`
      for (const shape of shapes) {
        html += `<button class="shape-btn" data-shape="${shape}">${SHAPE_LABELS[shape]}</button>`
      }
      html += `</div>`
      html += `</div>`
    }
  ...
  private createStarField() {
    ...
    for (let i = 0; i < dustCount; i++) {
      ...
      dustPos[i3 + 1] = r * Math.sin(phi) * Math.sin(theta) * 0.6
  ...
  private setupMouseControls() {
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      if (this.isRightDragging) {
        ...
        this.cameraElevation = Math.max(-1, Math.min(1, this.cameraElevation - e.movementY * 0.005))
      }
    })
    ...
    if (statusEl) {
      statusEl.textContent = '比耶: 移动光标/停留0.65秒选中 | 张手: 启动应用 | 握拳: 取消选中 | Tab: 面板'
      statusEl.style.color = 'rgba(255,255,255,0.6)'
    }
    if ($hints) {
      $hints.innerHTML = `
        <span>✌️ Peace 移动/选中</span>
        &nbsp;
        <span>🖐️ Palm 启动</span>
        &nbsp;
        <span>✊ Fist 取消</span>
  ...
  private switchShape(shape: ParticleShape) {
    ...
    if (indicator) {
      indicator.innerHTML = `<span class="cat-tag">${cat}</span>${SHAPE_LABELS[shape]}`
    }
  ...
  private handleHandSelect() {
    ...
    if (hit) {
      ...
      if (worldPos) {
        const nearest = this.ornamentSystem.findNearestOrnament(worldPos, 6.0)
        if (nearest) {
  ...
  private handleGestureForOrb(gesture: string) {
    ...
    if (gesture === 'open_palm') {
      if (selected) {
        ...
        // 无选中 → 相机仲裁：挂起 dwell 2 秒
        this.gestureCooldown.set(gesture, now)
        this.cameraControlActive = true
        if (this.cameraControlTimeout !== null) clearTimeout(this.cameraControlTimeout)
        this.cameraControlTimeout = window.setTimeout(() => {
          this.cameraControlActive = false
          this.cameraControlTimeout = null
          if (this.handCursorActive) this.applyCursorState('tracking')
        }, 2000)
        this.applyCursorState('suspended')
        if (statusEl) {
          statusEl.textContent = '🖐️ 相机控制中 (2s)...'
  ...
  // ========== 光标状态机（新增）==========
  private applyCursorState(state: 'hidden' | 'tracking' | 'armed' | 'dwelling' | 'locked' | 'suspended') {
    ...
    switch (state) {
      ...
      case 'tracking':
        this.cursorRing.style.display = 'block'
        this.cursorDot.style.display = 'block'
        this.cursorRing.style.opacity = '0.7'
        this.cursorDot.style.opacity = '0.7'
        this.cursorRing.style.borderColor = '#00ccff'
      ...
      case 'armed':
        ...
        this.cursorRing.style.borderColor = '#00ffcc'
        this.cursorRing.style.transform = 'translate(-50%, -50%) scale(1.15)'
        break
      case 'dwelling':
        this.cursorRing.style.display = 'block'
        this.cursorDot.style.display = 'block'
        this.cursorRing.style.opacity = '1'
        this.cursorDot.style.opacity = '1'
        this.cursorRing.style.borderColor = `hsl(${160 + this.dwellProgress * 40}, 100%, 60%)`
        this.cursorRing.style.transform = `translate(-50%, -50%) scale(${0.9 + this.dwellProgress * 0.3})`
        break
      case 'locked':
        this.cursorRing.style.display = 'block'
        this.cursorDot.style.display = 'block'
        this.cursorRing.style.opacity = '0.6'
        this.cursorDot.style.opacity = '0.6'
        this.cursorRing.style.borderColor = '#ff8800'
        this.cursorRing.style.transform = 'translate(-50%, -50%) scale(1)'
        break
      case 'suspended':
        this.cursorRing.style.display = 'block'
        ...
        this.cursorDot.style.opacity = '0.3'
        this.cursorRing.style.borderColor = '#8888aa'
  ...
  // ========== Dwell 停留选择逻辑（新增）==========
  private updateDwell(screenX: number, screenY: number) {
    this.raycaster.setFromCamera(this.mouseNDC, this.camera)
    const hitName = this.ornamentSystem.raycast(this.raycaster)
    const target = hitName ?? '__scene__'
    ...
    if (this.dwellTriggered) return
    const elapsed = performance.now() - this.dwellStartTime
    this.dwellProgress = Math.min(elapsed / this.DWELL_MS, 1)
    this.applyCursorState('dwelling')
    if (elapsed >= this.DWELL_MS) {
      this.triggerDwellSelection()
    }
  }
  ...
  private resetDwell() {
    ...
    this.dwellProgress = 0
    this.dwellTriggered = false
    if (this.cursorState === 'dwelling') {
      this.applyCursorState('tracking')
    }
  }

  private triggerDwellSelection() {
    this.dwellTriggered = true
    this.applyCursorState('locked')
    this.raycaster.setFromCamera(this.mouseNDC, this.camera)
    const hit = this.ornamentSystem.raycast(this.raycaster)
    const statusEl = document.getElementById('status')
    if (hit) {
    ...
    // 1.2 秒后解锁，允许下一次 dwell
    window.setTimeout(() => {
      this.dwellTriggered = false
      this.dwellTarget = null
      this.dwellProgress = 0
      if (!this.cameraControlActive && this.handCursorActive) {
        this.applyCursorState('tracking')
      }
    }, 1200)
  }
...
const GESTURE_NAMES: Record<import * as THREE from 'three'
import { EffectComposer } from 'three/examples/jsm/postprocessing/EffectComposer.js'
import { RenderPass } from 'three/examples/jsm/postprocessing/RenderPass.js'
import { UnrealBloomPass } from 'three/examples/jsm/postprocessing/UnrealBloomPass.js'
import { ShaderPass } from 'three/examples/jsm/postprocessing/ShaderPass.js'
